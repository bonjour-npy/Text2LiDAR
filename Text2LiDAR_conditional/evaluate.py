...
class Features10k(torch.utils.data.Dataset):
    def __init__(self, root, helper, train_reflectance, train_mask):
        self.sample_path_list = sorted(Path(root).glob("*.pth"))[:10_000]  # 测试 10k 个样本
        self.helper = helper
    ...
    def __getitem__(self, index):
        ...
        assert img.shape[0] == 5, img.shape  # 5 个通道：depth, xyz, reflectance
        depth = img[[0]]
        mask = torch.logical_and(depth > EVAL_MIN_DEPTH, depth < EVAL_MAX_DEPTH).float()  # 根据深度范围做掩码
        img = img * mask
...
def evaluate(args):
    ...
    preprocess_img = accelerator.prepare(
        preprocess_img
    )  # preprocess_img 是 rangenet 的预处理模块，继承自 nn.Module，也需要通过 accelerator.prepare 进行封装
    ...
    dataset = ds.load_dataset(
        path=f"data/{cfg.dataset}",  # nuLiDARText 数据集
...
if __name__ == "__main__":
    ...
    #     "--ckpt",
    #     type=Path,
    #     default="/project/r2dm-main/logs/diffusion/kitti_360/spherical-1024/transformer_400000/models/diffusion_0000400000.pth",
    # )
    # parser.add_argument(
    #     "--sample_dir",
    #     type=str,
    #     default="/project/r2dm-main/logs/diffusion/kitti_360/spherical-1024/transformer_400000/results",
    # )
    parser.add_argument(
        "--ckpt",
        type=Path,
